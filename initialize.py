...
def show_user_data(user_data: dict) -> None:
    ...
    print("Student ID:%s" % (user_data["std_id"]), "Password:%s" % (
        user_data["password"]), sep="\n")
    print("Your courses:")
    print("-"*20)
    print("Plan courses:")
    print("\n".join(map(lambda x: "Course %s_%s" %
          (x["course_id"], x["course_index"]), user_data["course"]["a"])))
    print("-"*20)
    print("Free courses:")
    print("\n".join(map(lambda x: "Course %s_%s" %
          (x["course_id"], x["course_index"]), user_data["course"]["b"])))
    print("-"*20)
...
def update(user_data: dict) -> dict:
    ...
    while True:
        while operate not in ["i", "p", "a", "b", "q"]:
            operate = input(
                "请输入更新指令(i.更改学号/p.更改密码/a.更改方案选课/b.更改自由选课/s.展示当前信息/q.保存并退出)(输入前面的字母即可):")
        if operate == "q":
            return user_data
        elif operate == "i":
            ...
            if new_passwd == input("Input the new password again:"):
                ...
                print(
                    "The passwords entered did not match.Failed to update the password!")
        elif operate in ["a", "b"]:
            order, args = "", []
            ...
            while True:
                print("%s:" % (en_name))
                print("\n".join(map(lambda x: "%d.%s" % (x[0], x[1]), enumerate(map(lambda x: "Course %s_%s" % (
                    x["course_id"], x["course_index"]), user_data["course"][operate]), 1))))
                while order not in ["a", "d", "q"]:
                    manage = input(
                        "请输入更改%s的指令(a [课程号] [课序号].添加新的课程/d [序号].删除第[序号]条课程/q.退出方案选课更新)(输入指令并带上参数):" % (name))
                    ...
                    if len(args) == 2:
                        try:
                            ...
                            if len(args[1]) != 2:
                                raise Exception
                            new_course = {
                                "course_id": args[0], "course_index": args[1]}
                            if new_course not in user_data["course"][operate]:
                                user_data["course"][operate].append(new_course)
                                print("Successfully added Course %s_%s!" %
                                      (args[0], args[1]))
                            else:
                                print("Course %s_%s has already existed!" %
                                      (args[0], args[1]))
                        except:
                    ...
                    if len(args) == 1:
                        try:
                            ...
                            try:
                                ...
                                print("Successfully deleted the Course %s_%s!" % (
                                    target["course_id"], target["course_index"]))
                            except:
                                print(
                                    "Invaild position!Failed to delete the course!")
                        except:
                            print(
                                "Invaild argument type!Failed to delete the course!")
                    else:
                        print("Invaild number of arguments!Failed to add the course!")
                order, args = "", []
        elif operate == "s":
